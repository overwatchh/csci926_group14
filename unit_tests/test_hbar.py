...
# -------- Function Under Test --------
def generate_horizontal_bar_chart(data, labels, title="Horizontal Bar Chart"):
...
def test_generate_chart_with_random_data(data, labels):
    fig = generate_horizontal_bar_chart(data, labels)
    buf = BytesIO()
    ...
    # Check y-ticks match labels
    yticklabels = [tick.get_text() for tick in fig.axes[0].get_yticklabels()]
    assert yticklabels == labels
    assert len(yticklabels) == len(data)


def test_chart_custom_title():
    data = [10, 20]
    labels = ["X", "Y"]
    title = "Custom HBar Title"
    fig = generate_horizontal_bar_chart(data, labels, title=title)
...
def test_chart_extreme_values(extreme_data, labels):
    ...
    yticklabels = [tick.get_text() for tick in fig.axes[0].get_yticklabels()]
    assert yticklabels == labels
    assert len(yticklabels) == len(extreme_data)


def test_chart_ylabels_and_bar_count():
    data = [5, 15, 25]
